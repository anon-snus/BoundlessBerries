...
class Transactions:
    ...
    async def send_token(self, token: types.Contract|types.Address|str, recipient: types.Address, amount: TokenAmount|int|float, nonce: int | None = None) -> Tx:
        """
        Отправить ERC-20 токен на указанный адрес.

        Args:
            token (Contract): Контракт токена.
            recipient (Address): Адрес получателя.
            amount (Amount): Количество токенов.
            gas_limit (Optional[GasLimit]): Лимит газа (если None, рассчитывается автоматически).
            nonce (Optional[int]): Nonce отправителя (если None, берётся автоматически).

        Returns:
            Tx: Объект отправленной транзакции.
        """
        recipient = Web3.to_checksum_address(recipient)
        token = await self.client.contracts.default_token(token)
        ...
        tx_params = {
            ...
            'to': token.address,
            'data': token.encodeABI('transfer', args=[recipient, amount_wei]),
            # Передаем два аргумента: адрес и количество
            # 'value': 0,
        }
        ...
        if receipt:
            return f'tx is successful: hash: {tx.hash.hex()}'
        return f'tx failed'
    ...
    @staticmethod
    async def parse_params(params: str, has_function: bool = True):
        i = 0
        if has_function:
            ...
            params = params[10:]
        while params:
            print(hex(i)[2:].zfill(3), ':', params[:64])
            params = params[64:]
            i += 32
